...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String _getInitials() {
    final source = first.isNotEmpty ? first : (displayName.isNotEmpty ? displayName : '');
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(Icons.home_outlined, size: 56, color: Theme.of(context).primaryColor),
          ),
  ...
  Widget _buildMySpaceTab() {
    ...
    // MySpaceScreen returns a Scaffold; embedding a Scaffold inside body is okay —
  ...
  Widget _buildCirclesTab() {
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 12, offset: const Offset(0, 6))]),
          padding: const EdgeInsets.all(18),
          child: Icon(Icons.groups_outlined, size: 56, color: Theme.of(context).primaryColor),
        ),
        const SizedBox(height: 14),
        Text('Circles', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
      ]),
    );
  }
    ...
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 12, offset: const Offset(0, 6))]),
          padding: const EdgeInsets.all(18),
          child: Icon(Icons.map_outlined, size: 56, color: Theme.of(context).primaryColor),
        ),
        const SizedBox(height: 14),
        Text('Map', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
      ]),
    );
  }
    ...
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 12, offset: const Offset(0, 6))]),
          padding: const EdgeInsets.all(18),
          child: Icon(Icons.receipt_long_outlined, size: 56, color: Theme.of(context).primaryColor),
        ),
        const SizedBox(height: 14),
        Text('Orders', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700)),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: ProfileAppBar(
        ...
        onNotificationsTap: () {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Notifications (placeholder)')));
        },
        onSettingsTap: () {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Settings (placeholder)')));
        },
      ),
